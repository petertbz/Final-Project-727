...
corpus_chrome_proc <-  tokens_replace(corpus_chrome_proc, # "Substitute token types based on vectorized one-to-one matching"
                                    lemmaData$V1, 
                                    lemmaData$V2,
                                    valuetype = "fixed") 
...
```{r}
# Drop rows that do not have content left

#sel_idx <- rowSums(DTM) > 0
# table(sel_idx) # check if there are blank rows. In Chrome's case, NO
#DTM <- DTM[sel_idx, ]
